...
last_wrench_print = 0.0
def wrench_cb(msg):
    global last_wrench_print, robot_is_moving

    # Only output wrench when robot is moving
    if not robot_is_moving:
        return

    now = time.time()

    # Throttle printing to once every 0.5 seconds
    if now - last_wrench_print < 0.5:
        return
    last_wrench_print = now
    ...
    print(f"[Wrench] F=({Fx:.2f}, {Fy:.2f}, {Fz:.2f})  "
...
_latest_tab = None
_tab_event = threading.Event()
_tab_lock = threading.Lock()

def pose_stamped_to_mat(msg: PoseStamped):
    """Convert PoseStamped -> 4x4 homogeneous numpy array T (rotation from quaternion, translation)."""
    ...
    rot = R.from_quat([qx, qy, qz, qw]).as_matrix()  # 3x3
    T = np.eye(4, dtype=float)
    T[:3, :3] = rot
    T[:3, 3] = [tx, ty, tz]
    return T

def tab_cb(msg: PoseStamped):
    global _latest_tab
    T = pose_stamped_to_mat(msg)
    print("Received /tab_pose:")
    print(T)     
    with _tab_lock:
        _latest_tab = T
        _tab_event.set()
    ros.loginfo("[nuc_simple] Received /tab_pose and stored Tab matrix.")
...
try:
    ...
    # Subscribe to wrench / force-torque from the real robot
    ros.Subscriber('/franka_state_controller/franka_states', FrankaState, wrench_cb)
    print("[INFO] Subscribed to wrench forces (/franka_state_controller/franka_states)")

except Exception:
    # node may already be running in this process; ignore
    pass

# Publish boolean to start_record topic to signal data recording
start_record_pub = ros.Publisher('/start_record', Bool, queue_size=1)

# subscribe to /tab_pose from PC
ros.Subscriber('/tab_pose', PoseStamped, tab_cb)
ros.loginfo("[q_goal_ros] Subscribed to /tab_pose, waiting for Tab messages from PC.")
...
print("Enter 6 values for target EE pose: x y z roll pitch yaw (radians) relative to arm frame")
...
Tea_world = SE3(x, y, z) * SE3.RPY([roll, pitch, yaw], order='xyz')
print("Desired target transform (Tea_world):")
print(Tea_world)

# Convert user goal into the robot BASE frame (used inside MMC loop)
#Tea = panda.base.inv() * Tea_world
# print("Converted Tep (base frame) used by MMC:")
Tea = Tea_world
print(Tea)
ros.loginfo("[q_goal_ros] Waiting for /tab_pose message to get Tab...")
_tab_event.clear()
if not _tab_event.wait(timeout=90.0):
    ros.logerr("[q_goal_ros] Timeout waiting for /tab_pose message. Exiting.")
    sys.exit(1)
with _tab_lock:
    Tab_mat = _latest_tab.copy()

Tab_se3 = SE3(Tab_mat)
print("Received Tab from /tab_pose:")
print(Tab_se3)
Tbe = Tab_se3.inv() * Tea.inv()
#Tbe = panda.base.inv() * Tbe
print("Computed Tbe for MMC loop:")
print(Tbe)
# Tbe = Tea * pose_stamped_to_mat(_latest_tab)
ros.loginfo("[q_goal_ros] Computed Tbe for MMC loop.")
print("Final Tbe used by MMC loop:")
print(Tbe)
...
try:
    while not arrived:
        ...
        err = np.sum(np.abs(np.r_[eTbe.t, eTbe.rpy() * np.pi / 180.0]))

        # End-effector spatial velocity to approach goal
        v, arrived = rtb.p_servo(Te, Tbe, 1.0)
